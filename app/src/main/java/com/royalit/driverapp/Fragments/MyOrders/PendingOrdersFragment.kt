...
class PendingOrdersFragment : Fragment() {
    ...
    private fun getOrdersListApi() {
        ...
        val call = apiServices.getOrdersListApi(getString(R.string.api_key),"2",stateDate,endDate)
    ...
    private fun dataSet(selectedOrdersList: List<OrderHistoryResponse>) {
        binding.recyclerview.apply {
            ...
            binding.recyclerview.adapter  = PendingOrdersListAdapter(requireActivity(), selectedOrdersList) { item ->
                val intent = Intent(requireActivity(), OrderDetailsActivity::class.java).apply {
                    putExtra("orderType", "Pending")
